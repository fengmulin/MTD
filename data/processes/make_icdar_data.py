...
class MakeICDARData(DataProcess2):
    ...
    def process(self, data):
        polygons = []
        ignore_tags = []
        annotations = data['polygon']
        for annotation in annotations:
            
            polygons.append(np.array(annotation['poly']))
            # polygons.append(annotation['points'])
            ignore = True if '#' in annotation['text'] else False
            ignore_tags.append(ignore)
            # print(annotation['text'], label)
