...
class MakeSegOriData(DataProcess2):
    r'''
    ...
    def process(self, data):
        ...
        for i in range(len(polygons)):
            ...
            #     ignore_tags[i] = True
                ...
                # print(polygon.shape)
                # raise
                cv2.drawContours(gt[0], [polygon.astype(np.int32)], -1, 1, -1)
                #cv2.fillPoly(gt[0], [polygon.astype(np.int32)], 1)
        # if gt[0].max()>1 or gt[0].min()<0: 
        # data_df = pd.DataFrame(gt[0])
        # print(gt[0].shape)
        # data_df.to_csv('pd_data5.csv')      
        # cv2.imwrite('qzz.png', gt[0]*255)
        # raise
        if filename is None:
            filename = ''
        gt[gt>1] = 1
        data.update(image=image,
                    polygons=polygons,
                    ori_gt=gt, ori_mask=mask, filename=filename)
        # print(data['ori_gt'])
        # raise
        return data
