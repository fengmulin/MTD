...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, 
        ...
        # self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        stage_with_dcn
        self.layer1 = self._make_layer(block, 64, layers[0], dcn= dcn if stage_with_dcn[0] else None)
        self.layer2 = self._make_layer(
            block, 128, layers[1], stride=2, dcn= dcn if stage_with_dcn[1] else None)
        self.layer3 = self._make_layer(
            block, 256, layers[2], stride=2, dcn= dcn if stage_with_dcn[2] else None)
        
        self.layer4 = self._make_layer(
            block, 512, layers[3], stride=2, dcn= dcn if stage_with_dcn[3] else None)
...
def df_res18_3(pretrained=True, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
...
def res50_3(pretrained=True, **kwargs):
...
def df_res50_3(pretrained=True, **kwargs):
