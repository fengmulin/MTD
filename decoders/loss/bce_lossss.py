...
class BalanceCrossEntropyLoss_my(nn.Module):
    ...
    def _bce_my_loss(self,pred,target):
        qzz = torch.full(pred.shape,-100.0).cuda()
        qzz2 = qzz = torch.full(pred.shape,-100.0).cuda()
        qzz.requires_grad=True
        qzz2.requires_grad=True
        # qzz.to('cuda').cuda()
        # qzz2.to('cuda')
        loss = (target - 1) * torch.max(
        torch.log(1 - pred), qzz
        ) - target * torch.max(torch.log(pred), qzz2)
        return loss
    def forward(self,
                pred: torch.Tensor,
                gt: torch.Tensor,
