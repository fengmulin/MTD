...
class DiceLoss(nn.Module):
    ...
    def _compute(self, pred, gt, mask, weights):
        ...
        input = pred.contiguous().view(batch_size, -1)
        target = gt.contiguous().view(batch_size, -1).float()
        ...
        b = torch.sum(input * input, dim=1) + 0.00001
        c = torch.sum(target * target, dim=1) + 0.00001
        d = (2 * a) / (b + c)
        loss = 1 - d
        return loss
