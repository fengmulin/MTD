...
class WarmupLR(Configurable):
    ...
    origin_lr = State()
    warmup_ratio = State(default=0.01)
    def __init__(self, cmd={}, **kwargs):
        self.load_all(**kwargs)

    def get_learning_rate(self, epoch, step):
        if step < self.steps:
            k = (1 - step / self.steps) * (1 -
                self.warmup_ratio)
            #warmup_lrs = [_lr * (1 - k) for _lr in warmup_lr]
            return self.warmup_lr * (1-k)
...
class DecayLearningRateStep(Configurable):
    lr = State(default=0.007)
    epochs = State(default=1200)
    ci = State(default=53671)
    factor = State(default=1.2)
    steps = State()

    def __init__(self, **kwargs):
        self.load_all(**kwargs)
        self.steps = self.ci*(self.epochs+1)

    def get_learning_rate(self, epoch, step=None, step_all=None):
        rate = np.power(1.0 - step / float(self.steps + 1), self.factor)
        return rate * self.lr
