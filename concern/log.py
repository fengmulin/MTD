...
class Logger(Configurable):
    ...
    cur_time = State(default=str(now)+'^')
    ...
    def __init__(self, **kwargs):
        ...
        self.name = cmd['name']
        # print(cmd['exp'])
        self.data_name = cmd['exp'].split('/')[-2]
        self.log_dir = os.path.join(self.data_name, self.cur_time  +self.name)
        #print(self.log_dir)
        self.log_dir = 'workspace/' + self.log_dir
        # print(self.log_dir)
        # raise
        if cmd['test']:
            parts = cmd['resume'].split('/')
            # print(cmd['resume'])
            # raise
            self.log_dir = '/'.join(parts[:3]) + '/test_res'
        # print(self.log_dir)
        # raise
    ...
    def save_image_dict(self, images, max_size=1024):
        for file_name, image in images.items():
            ...
            #     self.tf_board_logger.add_image(os.path.join(self.vis_dir(), file_name+'.jpg'), image.transpose(2,0,1))
            #     #continue
            # else:
            #     #print(image.shape)
            #     image = image[np.newaxis, :, :]
            #     #print(image.shape)
            #     self.tf_board_logger.add_image(os.path.join(self.vis_dir(), file_name+'.jpg'), image)
            if len(image.shape)==3:
